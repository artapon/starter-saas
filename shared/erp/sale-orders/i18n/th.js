export default {
  erp: {
    orders: {
      title:        'ใบสั่งขาย',
      new:          'ใบสั่งขายใหม่',
      searchPh:     'ค้นหาเลขที่ใบสั่ง…',
      noFound:      'ไม่พบใบสั่งขาย',
      allStatuses:  'ทุกสถานะ',
      draft:        'ร่าง',
      confirmed:    'ยืนยันแล้ว',
      shipped:      'จัดส่งแล้ว',
      delivered:    'ส่งถึงแล้ว',
      cancelled:    'ยกเลิก',
      customerInfo: 'ข้อมูลลูกค้าและคำสั่ง',
      customer:     'ลูกค้า',
      orderDate:    'วันที่สั่งขาย',
      taxRate:      'อัตราภาษี (%)',
      notes:        'หมายเหตุ',
      lineItems:    'รายการสินค้า',
      previewPrint:     'ดูตัวอย่างพิมพ์',
      journals:         'สมุดรายวัน',
      journalsCreateHint: 'รายการบัญชีจะปรากฏที่นี่เมื่อออกใบแจ้งหนี้หรือรับชำระเงินสำหรับใบสั่งขายนี้',
      journalsNone:      'ยังไม่มีรายการบัญชี',
      journalsNoneHint:  'การลงบัญชีใบแจ้งหนี้หรือใบเสร็จของใบสั่งขายนี้จะบันทึกรายการที่นี่',
      journalsPreviewBadge: 'ตัวอย่าง',
      journalsPreviewTitle: 'ประมาณการเมื่อออกใบแจ้งหนี้',
      journalsPreviewHint:  'ยังไม่ได้ลงบัญชี นี่คือรายการบัญชีที่จะถูกบันทึกเมื่อออกใบแจ้งหนี้สำหรับใบสั่งขายนี้',
      journalAccount:    'บัญชี',
      journalDebit:      'เดบิต',
      journalCredit:     'เครดิต',
      viewJournal:       'เปิดรายการบัญชี',
      addItem:      'เพิ่มรายการ',
      addFirstItem: 'เพิ่มรายการแรก',
      duplicateItemWarning: 'รายการนี้ปรากฏมากกว่าหนึ่งบรรทัด',
      dragToReorder: 'ลากเพื่อจัดเรียงใหม่',
      saleItem:     'รายการขาย',
      saleItems:    'รายการขาย',
      salePackage:  'แพ็กเกจ',
      salePackages: 'รายการขายแบบกลุ่ม',
      packageItem:  'รายการในแพ็กเกจ',
      removePackage: 'ลบแพ็กเกจและรายการทั้งหมด',
      collapsePackage: 'ย่อรายการในแพ็กเกจ',
      expandPackage:   'ขยายรายการในแพ็กเกจ',
      store:        'คลัง',
      description:  'คำอธิบาย',
      unitPrice:    'ราคาต่อหน่วย',
      lineDiscount: 'ส่วนลด',
      discountPercent: 'ส่วนลด %',
      discountAmount:  'จำนวนเงิน',
      amount:       'จำนวนเงิน',
      stock:           'คงเหลือ',
      availableStock:  'คงเหลือพร้อมขาย',
      qtyExceedsStock: 'จำนวนเกินสต็อกคงเหลือ',
      orderSummary: 'สรุปคำสั่ง',
      tax:          'ภาษี',
      vat:          'ภาษีมูลค่าเพิ่ม',
      wht:          'หัก ณ ที่จ่าย',
      netTotal:     'ยอดชำระสุทธิ',
      total:        'ยอดสุทธิ',
      subtotal:     'มูลค่าก่อน VAT',
      createOrder:  'สร้างใบสั่งขาย',
      editOrder:    'แก้ไขใบสั่ง',
      discard:      'ยกเลิก',
      readOnlyNotice: 'อ่านอย่างเดียว — แก้ไขได้เฉพาะใบสั่งที่เป็นร่าง',
      backToList:     'กลับไปที่รายการ',
      saveBeforeAction: 'บันทึกการเปลี่ยนแปลงก่อน',
      updateStatus: 'อัปเดตสถานะ',
      deleteOrder:  'ลบใบสั่ง',
      colCode:      'รหัส',
      colItem:      'รายการ',
      colQty:       'จำนวน',
      colUnitPrice: 'ราคาต่อหน่วย',
      colTotal:     'รวม',
      salesOrderInfo: 'ข้อมูลใบสั่งขาย',
      cancelOrder:    'ยืนยันยกเลิกใบสั่งขาย',
      confirmOrder:    'ยืนยันใบสั่งขาย',
      markAsShipped:   'ทำเครื่องหมายว่าจัดส่งแล้ว',
      markAsDelivered: 'ทำเครื่องหมายว่าส่งถึงแล้ว',
      alreadyLinkedTo: 'เชื่อมโยงกับ {ref} แล้ว',
      createDeliveryOrder: 'สร้างใบส่งสินค้า',
      createInvoice:       'สร้างใบแจ้งหนี้',

      // ── การยืนยันการดำเนินการ ──────────────────────────────
      confirm: {
        confirmed: { title: 'ยืนยันใบสั่งขายนี้?',       message: 'จะเปลี่ยนสถานะเป็น "ยืนยันแล้ว" และล็อกไม่ให้แก้ไขเพิ่มเติม' },
        shipped:   { title: 'ทำเครื่องหมายว่าจัดส่งแล้ว?', message: 'จะเปลี่ยนสถานะใบสั่งเป็น "จัดส่งแล้ว"' },
        delivered: { title: 'ทำเครื่องหมายว่าส่งถึงแล้ว?', message: 'จะเปลี่ยนสถานะใบสั่งเป็น "ส่งถึงแล้ว"' },
        cancelled: { title: 'ยกเลิกใบสั่งขายนี้?',        message: 'จะยกเลิกใบสั่งและไม่สามารถย้อนกลับได้' },
        convert:   { title: 'สร้างใบส่งสินค้า?',          message: 'ระบบจะสร้างใบส่งสินค้าจากใบสั่งขายนี้' },
      },

      // ── ประเภทการขาย (เงินสด / เครดิต) ─────────────────────
      saleType:        'ประเภทการขาย',
      saleTypeCash:    'เงินสด',
      saleTypeCredit:  'เครดิต',
      cashNextHint:    'ขายเงินสด — ออกใบเสร็จจากใบส่งสินค้า',
      creditNextHint:  'ขายเครดิต — ออกใบแจ้งหนี้จากใบส่งสินค้า',

      referenceNumber:   'เลขที่อ้างอิง / PO',
      expectedDelivery:  'วันที่ส่งที่คาดหวัง',
      paymentTerms:      'เงื่อนไขการชำระเงิน',
      salesperson:       'พนักงานขาย',
      addresses:         'ที่อยู่',
      shippingAddress:   'ที่อยู่จัดส่ง',
      billingAddress:    'ที่อยู่ใบกำกับ',
      sameAsShipping:    'ใช้ที่อยู่จัดส่ง',
      useCustomerAddress: 'ใช้ที่อยู่ของลูกค้า',
      discount:          'ส่วนลด',
      newCustomer:       'เพิ่มลูกค้าใหม่',

      // ── แจ้งเตือนลูกหนี้ค้างชำระ ─────────────────────────────
      arAlertTitle:      'ลูกหนี้ค้างชำระ',
      arOutstanding:     'ค้างชำระ',
      arViewReport:      'ดูรายงานอายุลูกหนี้ทั้งหมด',
      arViewInvoice:     'เปิดใบแจ้งหนี้ในแท็บใหม่',

      // ── ข้อความตัวอย่าง / คำแนะนำในช่องกรอก ─────────────────
      selectCustomerPh:  '— เลือกลูกค้า —',
      salespersonPh:     '— พนักงานขาย —',
      referencePh:       'เช่น PO-2025-001',
      shippingAddressPh: 'ที่อยู่จัดส่ง…',
      billingAddressPh:  'ที่อยู่ใบกำกับ…',
      customerNamePh:    'ชื่อลูกค้า',
      itemPh:            '— รายการ —',
      storePh:           '— คลัง —',
      itemSearchPh:      'ค้นหาด้วยรหัสหรือชื่อ…',
      addItemsHint:      'เพิ่มสินค้าหรือบริการในใบสั่งนี้',
      arAsOf:            'ณ วันที่',
      arDaysOverdue:     'เกินกำหนด {days} วัน',
      arNotOverdue:      'ยังไม่ครบกำหนด',

      saveDraft:         'บันทึกร่าง',
      savedDraft:        'บันทึกร่างแล้ว',
      dontSave:          'ไม่บันทึก',
      unsavedChanges:    'คุณมีการเปลี่ยนแปลงที่ยังไม่ได้บันทึก',
      unsavedChangesHint:'ต้องการออกจากหน้านี้และละทิ้งการเปลี่ยนแปลงหรือไม่?',
      fillRequiredFields:'กรุณากรอกข้อมูลที่จำเป็นก่อน',

      // ── Document view ──────────────────────────────────────
      documentTitle:           'ใบสั่งขาย',
      docOriginal:             'ต้นฉบับ',
      docDate:                 'วันที่',
      docPO:                   'เลขที่ PO',
      docBillTo:               'ออกบิลให้',
      docShipTo:               'จัดส่งให้',
      docSameAsBilling:        'ที่อยู่เดียวกับใบกำกับ',
      docPhoneAbbr:            'โทร.',
      docTaxId:                'เลขประจำตัวผู้เสียภาษี',
      docCustomerCode:         'รหัสลูกค้า',
      docCustomerName:         'ชื่อลูกค้า',
      docAddress:              'ที่อยู่',
      docOrderNo:              'เลขที่ใบสั่งขาย',
      docExpectedDelivery:     'วันที่ส่ง',
      docVat:                  'ภาษีมูลค่าเพิ่ม',
      docNetTotal:             'ยอดสุทธิ',
      docTerm1:                'ใบสั่งขายนี้ยืนยันรายการและราคาตามที่ระบุไว้ข้างต้น',
      docTerm2:                'วันที่จัดส่งเป็นการประมาณการและอาจมีการเปลี่ยนแปลง',
      docPreparedBy:           'ผู้จัดทำ',
      docApprovedBy:           'ผู้อนุมัติ',
      docAuthorisedSignature:  'ผู้มีอำนาจลงนาม',
      docCustomerSignature:    'ลูกค้าลงนาม',
      docFooterThanks:         'ขอบคุณที่ใช้บริการ',
      printDocument:           'พิมพ์เอกสารนี้',
      nextAction:              'การดำเนินการต่อไป',
      nextActionHint:          'เลื่อนสถานะใบสั่งไปยังขั้นถัดไป',
      convertActions:          'สร้าง',
    },

    orderItems: {
      title:          'รายการในใบสั่ง',
      editItem:       'แก้ไขรายการ',
      itemMasterLink: 'ลิงก์ข้อมูลหลัก',
      description:    'คำอธิบาย',
      quantity:       'จำนวน',
      unitPrice:      'ราคาต่อหน่วย',
      deleteItem:     'ลบรายการ',
      colProduct:     'สินค้า / คำอธิบาย',
      colOrderNo:     'เลขที่ใบสั่ง',
      colQty:         'จำนวน',
      colPrice:       'ราคา',
      colStockLink:   'ลิงก์สต็อก',
      colOrderStatus: 'สถานะใบสั่ง',
      colActions:     'จัดการ',
    },

    deliveryOrders: {
      title:            'ใบส่งสินค้า',
      new:              'ใบส่งสินค้าใหม่',
      create:           'สร้างใบส่งสินค้า',
      editOrder:        'แก้ไขใบส่งสินค้า',
      info:             'ข้อมูลการจัดส่ง',
      searchPh:         'ค้นหาเลขที่อ้างอิง…',
      noFound:          'ไม่พบใบส่งสินค้า',
      notFound:         'ไม่พบใบส่งสินค้า',
      customer:         'ลูกค้า',
      deliveryDate:     'วันที่กำหนดส่ง',
      referenceSO:      'อ้างอิงใบสั่งขาย',
      refSO:            'เลขที่ใบสั่งขาย',
      deliveryAddress:  'ที่อยู่จัดส่ง',
      deliverySummary:  'สรุป',
      totalItems:       'จำนวนรายการ',
      lineItems:        'รายการสินค้า',
      noItemsHint:      'เพิ่มสินค้าที่ต้องการจัดส่ง',
      addItem:          'เพิ่มรายการ',
      addFirstItem:     'เพิ่มรายการแรก',
      saleItem:         'รายการขาย',
      saleItems:        'รายการขาย',
      salePackages:     'รายการขายแบบกลุ่ม',
      store:            'คลัง',
      loadFromSO:       'นำเข้าจากใบสั่งขาย',
      colCode:          'รหัส',
      colRefNo:         'เลขที่อ้างอิง',
      colCustomer:      'ลูกค้า',
      colDate:          'วันที่',
      colDeliveryDate:  'วันที่ส่ง',
      colProduct:       'สินค้า',
      colNotes:         'หมายเหตุ',
      colQty:           'จำนวน',
      cancelOrder:      'ยกเลิกใบส่งสินค้า',
      deleteOrder:      'ลบใบส่งสินค้า',
      confirmOrder:     'ยืนยันใบส่งสินค้า',
      markShipped:      'บันทึกการจัดส่ง',
      markDelivered:    'บันทึกการส่งมอบ',
      statusShipped:    'กำลังจัดส่ง',
      statusDelivered:  'ส่งมอบแล้ว',
      customerRequired: 'กรุณาเลือกลูกค้า',
      dateRequired:     'กรุณาระบุวันที่',
      createInvoice:    'สร้างใบแจ้งหนี้',
      createReceipt:    'สร้างใบเสร็จ',

      // ── Sales-order parity ─────────────────────────────────
      referenceNumber:   'เลขที่อ้างอิง / PO',
      paymentTerms:      'เงื่อนไขการชำระเงิน',
      salesperson:       'พนักงานขาย',
      addresses:         'ที่อยู่',
      shippingAddress:   'ที่อยู่จัดส่ง',
      billingAddress:    'ที่อยู่ใบกำกับ',
      sameAsShipping:    'ใช้ที่อยู่จัดส่ง',
      useCustomerAddress:'ใช้ที่อยู่ของลูกค้า',
      discard:           'ยกเลิก',
      unsavedChanges:    'คุณมีการเปลี่ยนแปลงที่ยังไม่ได้บันทึก',
      unsavedChangesHint:'ต้องการออกจากหน้านี้และละทิ้งการเปลี่ยนแปลงหรือไม่?',
      fillRequiredFields:'กรุณากรอกข้อมูลที่จำเป็นก่อน',

      // ── Document view ──────────────────────────────────────
      documentTitle:           'ใบส่งสินค้า',
      docOriginal:             'ต้นฉบับ',
      docDate:                 'วันที่',
      docPO:                   'เลขที่ PO',
      docBillTo:               'ออกบิลให้',
      docShipTo:               'จัดส่งให้',
      docSameAsBilling:        'ที่อยู่เดียวกับใบกำกับ',
      docPhoneAbbr:            'โทร.',
      docTaxId:                'เลขผู้เสียภาษี:',
      docCustomerCode:         'รหัสลูกค้า',
      docCustomerName:         'ชื่อลูกค้า',
      docAddress:              'ที่อยู่',
      docRefNo:                'เลขที่ใบส่งสินค้า',
      docDeliveryDate:         'วันที่ส่ง',
      docTerm1:                'ได้รับสินค้าในสภาพเรียบร้อยและจำนวนถูกต้อง',
      docTerm2:                'กรุณาตรวจสอบสินค้าก่อนลงนามรับในใบส่งสินค้านี้',
      docPreparedBy:           'ผู้จัดเตรียม',
      docApprovedBy:           'ผู้อนุมัติ',
      docAuthorisedSignature:  'ผู้มีอำนาจลงนาม',
      docReceiverSignature:    'ผู้รับสินค้าลงนาม',
      docFooterThanks:         'ขอบคุณที่ใช้บริการ',
      printDocument:           'พิมพ์เอกสารนี้',
      nextAction:              'การดำเนินการต่อไป',
      nextActionHint:          'เลื่อนสถานะใบส่งสินค้าไปยังขั้นถัดไป',
      convertActions:          'สร้าง',
    },
  },
}
